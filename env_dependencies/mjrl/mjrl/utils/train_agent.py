...
def _load_latest_policy_and_logs(agent, *, policy_dir, logs_dir):
    ...
    i = last_step
    while i >= 0:
        policy_path = os.path.join(policy_dir, f"policy_{i}.pickle")
        baseline_path = os.path.join(policy_dir, f"baseline_{i}.pickle")

        if not os.path.isfile(policy_path):
            i = i - 1
            continue
        else:
            print(f"Loaded last saved iteration: {i}")
...
def train_agent(
    ...
    for i in range(i_start, niter):
        ...
        args = dict(
            N=N, sample_mode=sample_mode, gamma=gamma, gae_lambda=gae_lambda, num_cpu=num_cpu, env_kwargs=env_kwargs
        )
        ...
        print(
            "[ %s ] %4i %5.2f %5.2f %5.2f "
            % (timer.asctime(timer.localtime(timer.time())), i, train_curve[i], mean_pol_perf, best_perf)
        )
