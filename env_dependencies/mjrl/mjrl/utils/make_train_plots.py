...
def make_train_plots(
    log=None, log_path=None, keys=None, save_loc=None, sample_key="num_samples", x_scale=1.0, y_scale=1.0
):
    ...
    for key in keys:
        if key in log.keys():
            ...
            try:
                ...
                ax2 = ax1.twiny()
                ax2.set_xlabel("iterations", color=(0.7, 0.7, 0.7))
                ax2.tick_params(axis="x", labelcolor=(0.7, 0.7, 0.7))
...
def main():
    ...
    parser.add_argument("-l", "--log_path", type=str, required=True, help="path file to log.csv")
    parser.add_argument("-k", "--keys", type=str, action="append", nargs="+", required=True, help="keys to plot")
    parser.add_argument("-s", "--save_loc", type=str, default="", help="Path for logs")
