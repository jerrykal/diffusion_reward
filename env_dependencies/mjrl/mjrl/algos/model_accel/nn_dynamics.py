...
class WorldModel:
    ...
    def fit_dynamics(self, s, a, sp, fit_mb_size, fit_epochs, max_steps=1e4, set_transformations=True, *args, **kwargs):
        ...
        return_vals = fit_model(
            self.dynamics_net, X, Y, self.dynamics_opt, self.dynamics_loss, fit_mb_size, fit_epochs, max_steps=max_steps
        )
        self.dynamics_net._apply_out_transforms = True
        return return_vals

    def fit_reward(self, s, a, r, fit_mb_size, fit_epochs, max_steps=1e4, set_transformations=True, *args, **kwargs):
        if not self.learn_reward:
        ...
        Y = r
        return fit_model(
            self.reward_net, X, Y, self.reward_opt, self.reward_loss, fit_mb_size, fit_epochs, max_steps=max_steps
        )
...
class DynamicsNet(nn.Module):
    ...
    def set_transformations(
        self, s_shift=None, s_scale=None, a_shift=None, a_scale=None, out_shift=None, out_scale=None
    ):
        if s_shift is None:
    ...
    def get_params(self):
        ...
        transforms = (self.s_shift, self.s_scale, self.a_shift, self.a_scale, self.out_shift, self.out_scale)
