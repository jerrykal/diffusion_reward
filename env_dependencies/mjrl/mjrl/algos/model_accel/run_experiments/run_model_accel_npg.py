...
models = [
    WorldModel(state_dim=e.observation_dim, act_dim=e.action_dim, seed=SEED + i, **job_data)
...
for outer_iter in range(job_data["num_iter"]):
    ...
    if job_data["refresh_fit"]:
        models = [
            WorldModel(state_dim=e.observation_dim, act_dim=e.action_dim, seed=SEED + 123 * outer_iter, **job_data)
            for i in range(job_data["num_models"])
        ]
