...
from mjrl.algos.model_accel.sampling import evaluate_policy, sample_paths
...
e.set_seed(SEED)
models = [
    DynamicsModel(state_dim=e.observation_dim, act_dim=e.action_dim, seed=SEED + i, **job_data)
...
for outer_iter in range(job_data["num_iter"]):
    ...
    if outer_iter == 0:
        iter_paths = trajectory_sampler.sample_paths(
            job_data["n_init_paths"], e, exploratory_policy, eval_mode=False, base_seed=SEED
        )
    else:
    ...
    if job_data["refresh_fit"]:
        models = [
            DynamicsModel(state_dim=e.observation_dim, act_dim=e.action_dim, seed=SEED + 123 * outer_iter, **job_data)
            for i in range(job_data["num_models"])
        ]
    ...
    make_train_plots(
        log=logger.log, keys=["rollout_score", "eval_score", "rollout_metric", "eval_metric"], save_loc=OUT_DIR + "/"
    )
