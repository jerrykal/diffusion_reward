...
class TrainVQGAN:
        ...
        for epoch in range(args.epochs):
            ...
            with tqdm(range(len(train_dataset))) as pbar:
                for i, imgs in zip(pbar, train_dataset):
                    ...
                    perceptual_rec_loss = (
                        args.perceptual_loss_factor * perceptual_loss + args.rec_loss_factor * rec_loss
                    )
                    ...
                    if i % 100 == 0:
                        with torch.no_grad():
                            ...
                            vutils.save_image(
                                real_fake_images,
                                os.path.join(f"{self.work_dir}/visualization/epoch{epoch}", f"{i}.jpg"),
                                nrow=4,
                            )
                ...
                if is_best:
                    torch.save(self.vqgan.state_dict(), os.path.join(f"{self.work_dir}/checkpoints", "vqgan.pt"))
    ...
    def eval(self, val_dataset, step):
        ...
        for imgs in val_dataset:
            ...
            rec_loss = torch.abs(imgs - decoded_images)
            perceptual_rec_loss = (
                self.args.perceptual_loss_factor * perceptual_loss + self.args.rec_loss_factor * rec_loss
            )
            perceptual_rec_loss = perceptual_rec_loss.mean()
            g_loss = -torch.mean(disc_fake)
