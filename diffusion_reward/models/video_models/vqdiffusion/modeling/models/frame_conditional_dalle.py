        ...
        **kwargs,
        ...
        for fr in filter_ratio:
            for cr in content_ratio:
                ...
                content_samples[f"cond1_cont{cr}_fr{fr}_image"] = self.content_codec.decode(trans_out["content_token"])

                if return_att_weight:
                    content_samples[f"cond1_cont{cr}_fr{fr}_image_condition_attention"] = trans_out[
                        "condition_attention"
                    ]  # B x Lt x Ld
                    content_att = trans_out["content_attention"]
                    shape = *content_att.shape[:-1], self.content.token_shape[0], self.content.token_shape[1]
                    content_samples[f"cond1_cont{cr}_fr{fr}_image_content_attention"] = content_att.view(
                        *shape
