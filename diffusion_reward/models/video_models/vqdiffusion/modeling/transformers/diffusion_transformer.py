...
class DiffusionTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.prior_rule = (
            0  # inference rule: 0 for VQ-Diffusion v1, 1 for only high-quality inference, 2 for purity prior
        )
    ...
    @torch.no_grad()
    def p_sample(
        self, log_x, cond_emb, t, sampled=None, to_sample=None
        ...
        if (
            ...
            for i in range(log_x.shape[0]):
                ...
                sampled[i] += (
                    (out2_idx[i] != self.num_classes - 1).sum() - (log_x_idx[i] != self.num_classes - 1).sum()
                ).item()
    ...
    def log_sample_categorical(
        self, logits, noise=True, noise_scale=1
    ...
    def forward(self, input, return_loss=False, return_logits=True, return_att_weight=False, is_train=True, **kwargs):
        ...
        if self.condition_emb is not None:
            with autocast(enabled=False):
                ...
                if self.learnable_cf:
                    is_empty_text = (
                        torch.logical_not(input["condition_mask"][:, 2]).unsqueeze(1).unsqueeze(2).repeat(1, 77, 512)
                    )
                    cond_emb = torch.where(
                        is_empty_text,
                        self.empty_text_embed.unsqueeze(0).repeat(cond_emb.shape[0], 1, 1),
                        cond_emb.type_as(self.empty_text_embed),
                    )
                ...
                for diffusion_index in range(start_step - 1, -1, -1):
                    ...
                    while min(sampled) < self.n_sample[diffusion_index]:
                        log_z, sampled = self.p_sample(
                            log_z, cond_emb, t, sampled, self.n_sample[diffusion_index]
                        )  # log_z is log_onehot
