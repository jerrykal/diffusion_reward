...
class DalleMaskImageEmbedding(BaseEmbedding):
    ...
    def forward(self, index, **kwargs):
        ...
        try:
            ...
            raise RuntimeError(
                f"IndexError: index out of range in self, max index {index.max()}, num embed {self.num_embed}"
            )
        ...
        if emb.shape[1] > 0:
            ...
            if self.pos_emb_type == "embedding":
                height_emb = self.height_emb(
                    torch.arange(self.spatial_size[0], device=index.device).view(1, self.spatial_size[0])
                ).unsqueeze(2)  # 1 x H x D -> 1 x H x 1 x D
                width_emb = self.width_emb(
                    torch.arange(self.spatial_size[1], device=index.device).view(1, self.spatial_size[1])
                ).unsqueeze(1)  # 1 x W x D -> 1 x 1 x W x D
            else:
