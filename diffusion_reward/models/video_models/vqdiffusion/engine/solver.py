...
from ..utils.misc import format_seconds, get_model_parameters_info, instantiate_from_config
...
class Solver:
    def __init__(self, config, args, model, dataloader, logger):
        ...
        if adjust_lr == "none":
            ...
            raise NotImplementedError(f"Unknown type of adjust lr {adjust_lr}!")
        self.logger.log_info(f"Get lr {self.lr} from base lr {base_lr} with {adjust_lr}")

        if hasattr(model, "get_optimizer_and_scheduler") and callable(model.get_optimizer_and_scheduler):
        ...
        if self.args.distributed:
            ...
            self.model = torch.nn.parallel.DistributedDataParallel(
                self.model, device_ids=[self.args.gpu], find_unused_parameters=False
            )
        ...
        self.logger.log_info(
            f"{self.args.exp_name}: global rank {self.args.global_rank}: prepare solver done!", check_primary=False
        )
    ...
    def _get_optimizer_and_scheduler(self, op_sc_list):
        ...
        for op_sc_cfg in op_sc_list:
            ...
            op_sc["optimizer"] = {"module": optimizer, "step_iteration": op_cfg.get("step_iteration", 1)}
            assert isinstance(op_sc["optimizer"]["step_iteration"], int), (
                "optimizer steps should be a integer number of iterations"
            )
            ...
            if "scheduler" in op_sc_cfg:
                ...
                op_sc["scheduler"] = {"module": scheduler, "step_iteration": sc_cfg.get("step_iteration", 1)}
    ...
    def _get_lr(self, return_type="str"):
        ...
        if return_type == "str":
            ...
            raise ValueError(f"Unknow of return type: {return_type}")
        return lrs
    ...
    def sample(self, batch, phase="train", step_type="iteration"):
            ...
            for k, v in samples.items():
                ...
                if torch.is_tensor(v) and v.dim() == 4 and v.shape[1] in [1, 3]:  # image
                    ...
                    self.logger.add_images(
                        tag="{}/{}e_{}itr/{}".format(
                            phase, self.last_epoch, self.last_iter % self.dataloader["train_iterations"], k
                        ),
                        img_tensor=im,
                        global_step=step,
                        dataformats="NCHW",
        ...
        self.logger.log_info(f"Sample done, time: {time.time() - tic:.2f}")
        ...
        for op_sc_n, op_sc in self.optimizer_and_scheduler.items():
            ...
            if phase == "train":
                if (
                    ...
                    and (self.last_iter + 1) % op_sc["optimizer"]["step_iteration"] == 0
                ):
                ...
                if "scheduler" in op_sc:
                    if (
                        ...
                        and (self.last_iter + 1) % op_sc["scheduler"]["step_iteration"] == 0
                    ):
    ...
    def save(self, force=False, is_best=False):
        if is_primary():
            ...
            if save or force:
                ...
                if save:
                    save_path = os.path.join(
                        self.ckpt_dir, f"{str(self.last_epoch).zfill(6)}e_{self.last_iter}iter.pth"
                    )
                    torch.save(state_dict, save_path)
                    self.logger.log_info(f"saved in {save_path}")
            ...
            if is_best:
                ...
                torch.save(state_dict, save_path)
                self.logger.log_info(f"best checktpoint saved in {save_path} !!!!!!!!!!")
    ...
    def resume(
        ...
        if os.path.exists(path):
            state_dict = torch.load(path, map_location=f"cuda:{self.args.local_rank}")
            ...
            self.logger.log_info(f"Resume from {path}")
    ...
    def train_epoch(self):
        ...
        for itr, batch in enumerate(self.dataloader["train_loader"]):
            ...
            if self.logger is not None and self.last_iter % self.args.log_frequency == 0:
                info = f"{self.args.exp_name}: train"
                ...
                for loss_n, loss_dict in loss.items():
                    ...
                    info += "" if loss_n == "none" else f" {loss_n}"
                    ...
                    for k in loss_dict:
                        info += f" | {k}: {float(loss_dict[k]):.4f}"
                        self.logger.add_scalar(
                            tag=f"train/{loss_n}/{k}", scalar_value=float(loss_dict[k]), global_step=self.last_iter
                        )
                ...
                for k in lrs.keys():
                    lr = lrs[k]
                    self.logger.add_scalar(tag=f"train/{k}_lr", scalar_value=lrs[k], global_step=self.last_iter)
                ...
                info += " || data_time: {dt}s | fbward_time: {fbt}s | iter_time: {it}s | iter_avg_time: {ita}s | epoch_time: {et} | spend_time: {st} | left_time: {lt}".format(
                    ...
                    lt=format_seconds(
                        itr_time_avg * self.max_epochs * self.dataloader["train_iterations"] - spend_time
                    ),
                )
        ...
        if val:
            ...
            for itr, batch in enumerate(self.dataloader["validation_loader"]):
                ...
                if self.logger is not None and (itr + 1) % self.args.log_frequency == 0:
                    info = f"{self.args.exp_name}: val"
                    info = info + ": Epoch {}/{} | iter {}/{}".format(
                        self.last_epoch, self.max_epochs, itr, self.dataloader["validation_iterations"]
                    )
                    for loss_n, loss_dict in loss.items():
                        info += " ||"
                        info += "" if loss_n == "none" else f" {loss_n}"
            ...
            if self.logger is not None:
                ...
                for loss_n, loss_dict in overall_loss.items():
                    info += "" if loss_n == "none" else f" {loss_n}"
                    info += f": Epoch {self.last_epoch}/{self.max_epochs}"
                    for k in loss_dict:
                        info += f" | {k}: {float(loss_dict[k]):.4f}"
                        self.logger.add_scalar(
                            tag=f"val/{loss_n}/{k}", scalar_value=float(loss_dict[k]), global_step=self.last_epoch
                        )
                self.logger.log_info(info)
    ...
    def train(self):
        ...
        self.logger.log_info(
            f"{self.args.exp_name}: global rank {self.args.global_rank}: start training...", check_primary=False
        )
