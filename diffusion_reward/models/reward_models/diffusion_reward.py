        ...
        if self.reward_type == "entropy":
            ...
            for _ in range(self.num_sample):
                ...
                for diffusion_index in diffusion_list:
                    ...
                    if diffusion_index > self.skip_step:
                        model_log_prob = transformer.q_posterior(
                            log_x_start=log_x_recon, log_x_t=log_z, t=t - self.skip_step
                        )
                    ...
                    log_z = transformer.log_sample_categorical(
                        model_log_prob, noise=self.noise, noise_scale=self.noise_scale
                    )
            ...
            for diffusion_index in diffusion_list:
                ...
                if diffusion_index > self.skip_step:
                    ...
                    log_true_prob = transformer.q_posterior(
                        log_x_start=log_x_start, log_x_t=log_z, t=t - self.skip_step
                    )
                else:
                    model_log_prob = transformer.q_posterior(log_x_start=log_x_recon, log_x_t=log_z, t=t)
