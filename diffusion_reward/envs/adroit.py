        ...
        if self.encoder is not None:
            for cam in self.cameras:
                img = self._env.env.sim.render(
                    width=self.width, height=self.height, mode="offscreen", camera_name=cam, device_id=0
                )
    ...
    def __init__(
        ...
        if env_feature_type == "pixels":
            self._obs_spec = specs.BoundedArray(
                shape=(number_channel, image_size[0], image_size[1]),
                dtype="uint8",
                name="observation",
                minimum=0,
                maximum=255,
            ...
            self._obs_spec = specs.Array(
                shape=(512 * num_frames * len(cam_list),), dtype="float32", name="observation"
            )  # TODO fix magic number
            self._obs_sensor_spec = specs.Array(
                shape=(self.obs_sensor_dim,), dtype="float32", name="observation_sensor"
            )
        self._action_spec = specs.BoundedArray(
            shape=(self.act_dim,), dtype="float32", name="action", minimum=-1.0, maximum=1.0
        )
